...
static int sm8150_get_batt_health(struct bms_dev *bms)
{
...
static int sm8150_get_batt_iterm(struct bms_dev *bms)
{
	int rc, temp;
	u8 stat, buf[2];
	...
	rc = sm8150_read(bms->pmic_regmap, CHGR_ADC_ITERM_UP_THD_MSB,
			 buf, 2);
	if (rc < 0) {
...
static int sm8150_get_batt_present(struct bms_dev *bms)
{
	int rc, ret;
	u8 stat;
				       ...
				       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		pval->intval = sm8150_get_chg_status(bms, &dc_valid,
						      &usb_valid);
		break;
	...
	case POWER_SUPPLY_PROP_HEALTH:
		pval->intval = sm8150_get_batt_health(bms);
		break;
	case POWER_SUPPLY_PROP_RECHARGE_SOC:
		pval->intval = bms->rl_soc_threshold;
		break;
	case POWER_SUPPLY_PROP_CHARGE_TERM_CURRENT:
		pval->intval = sm8150_get_batt_iterm(bms);
		break;
	case POWER_SUPPLY_PROP_PRESENT:
		pval->intval = sm8150_get_batt_present(bms);
		break;
	default:
