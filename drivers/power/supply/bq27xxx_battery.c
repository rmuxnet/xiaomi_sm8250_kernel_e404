...
enum bq27xxx_reg_index {
	...
	BQ27XXX_REG_CHG,	/* State-of-Charge */
	BQ27XXX_REG_DCAP,	/* Design Capacity */
	BQ27XXX_REG_CUR,	/* Design Capacity */
...
static u8
	bq27000_regs[BQ27XXX_REG_MAX] = {
		...
		[BQ27XXX_REG_CHG] = INVALID_REG_ADDR,
		[BQ27XXX_REG_DCAP] = 0x76,
		[BQ27XXX_REG_CUR] = 0x32,
		[BQ27XXX_REG_AP] = 0x24,
	...
	union power_supply_propval *val_status)
{
	...
	curr = bq27xxx_read(di, BQ27XXX_REG_CN, false);
	if (curr < 0) {
...
/*
 * Return the maximum charging current battery in miliamps
 * Or < 0 if something fails.
 */
static int bq27xxx_charge_current(struct bq27xxx_device_info *di,
				   union power_supply_propval *val)
{
	int max_current;

	max_current = bq27xxx_read(di, BQ27XXX_REG_CUR, false);
	if (max_current < 0) {
...
/*
 * Return the average charging current battery in miliamps
 * Or < 0 if something fails.
 */
static int bq27xxx_current_avg(struct bq27xxx_device_info *di,
				   union power_supply_propval *val)
{
	int max_current;

	max_current = bq27xxx_read(di, BQ27XXX_REG_AI, false);
	if (max_current < 0) {
		dev_err(di->dev, "error reading voltage\n");
		return max_current;
	}
...
/*
 * Return the max battery Voltage in millivolts
 * Or < 0 if something fails.
 */
static int bq27xxx_battery_voltage_design(struct bq27xxx_device_info *di,
				   union power_supply_propval *val)
{
	int volt;

	volt = bq27xxx_read(di, BQ27XXX_REG_CHG, false);
	if (volt < 0) {
		dev_err(di->dev, "error reading voltage\n");
		return volt;
	}
 ...
 * Or < 0 if something fails.
 */
static int bq27xxx_read_soh(struct bq27xxx_device_info *di,
				   union power_supply_propval *val)
{
	int health;

	health = bq27xxx_read(di, BQ27XXX_REG_DCAP, false);
	if (health < 0) {
					...
					union power_supply_propval *val)
{
...
#if IS_ENABLED(CONFIG_GOOGLE_BMS)
	if (!di->bms_psy) {
		di->bms_psy = power_supply_get_by_name("sm8150_bms");
	}
#endif
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX_DESIGN:
		ret = bq27xxx_battery_voltage_design(di, val);
		break;
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		ret = bq27xxx_charge_current(di, val);
		break;
	case POWER_SUPPLY_PROP_CURRENT_AVG:
		ret = bq27xxx_current_avg(di, val);
		break;
...
#if IS_ENABLED(CONFIG_GOOGLE_BMS)
		if (di->bms_psy)
			ret = power_supply_get_property(di->bms_psy,
